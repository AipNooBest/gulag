...
async def osuSearchHandler(
    ...
    search_url = f"{app.settings.MIRROR_URL}/api/v2/search"
    ...
    for bmap in result:
        if bmap["beatmaps"] is None:
            continue

        bmap["video"] = int(bmap["video"])

        diff_sorted_maps = sorted(
            bmap["beatmaps"],
            key=lambda m: m["difficulty_rating"],
        )
        diffs_str = ",".join(
